#!/usr/bin/env python3
"""Parse a manually marked quiz text file back into a user answers JSON (moved to scripts/)."""
from __future__ import annotations
import re, argparse, json
from pathlib import Path
from typing import Dict
QUESTION_RE = re.compile(r'^(Q\d+):')
OPTION_RE = re.compile(r'^\[(?P<mark>[xX ])\]\s+([A-D])\.\s')

def parse_args(argv):
    p = argparse.ArgumentParser(description='Parse marked quiz text to JSON answers')
    p.add_argument('--in', dest='inp', default='quiz.txt')
    p.add_argument('--out', dest='out', default='my_answers.json')
...
def main(argv=None):
    ...
    in_path = Path(args.inp); out_path = Path(args.out)
    ...
    with in_path.open(encoding='utf-8') as f:
        for line in f:
            ...
            if om and current_qid:
                marked = om.group('mark').lower() == 'x'
                letter = line.split()[1].rstrip('.').upper()
                current_marks.append((letter, marked))
        finalize()
    out_path.write_text(json.dumps(answers, indent=2) + '\n', encoding='utf-8')
    print(f"[ok] Wrote answers JSON -> {out_path} ({len(answers)} answered)"); return 0
