#!/usr/bin/env python3
"""Export quiz.json into a plain text template for manual marking (moved to scripts/)."""
...
def parse_args(argv):
    ...
    p.add_argument('--out', default='quiz.txt')
...
def main(argv=None):
    ...
    quiz_path = Path(args.quiz); out_path = Path(args.out)
    if not quiz_path.exists(): print(f"[error] quiz file {quiz_path} not found"); return 1
    if out_path.exists() and not args.force: print(f"[warn] {out_path} already exists (use --force to overwrite)"); return 1
    ...
    for q in quiz:
        ...
        lines.append('')
    out_path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f"[ok] Wrote markable quiz template -> {out_path}"); return 0
